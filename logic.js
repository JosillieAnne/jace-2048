...
//These variables monitors if player wins 
//If one of these variables value became true, the player won
	...
	for(let i = 0; i < row.length - 1; i++){
        ...
        */
        ...
        if(row[i] == row[i+1]){
            ...
            row[i+1] = 0;      
            score += row[i]; // This code is for the score 
        } // [2, 2, 2] -> [4, 0, 2]
...
function slideLeft(){
	...
	for(let r=0; r<rows; r++){
		...
		// all tiles values per row are saved in container
		let row = board[r];

		//line for animation
		let originalRow = row.slice(); //initial position of row


		// we used slide function to merge tiles with same values
		row = slide(row);

		// update the row with the merged tile
		board[r] = row;

		// because of this loop, we are able  to update the ids and color of all the tiles
		for(let c = 0; c<columns; c++){
...
//for merging slideUp and slideDown mainly on column amu na una ang column sa loop unlike sa slideright and slide left
function slideUp(){
	for(let c=0; c<columns; c++){
		...
		//update the id and color of tile
		for(let r=0; r<rows; r++){
			board[r][c] = row[r];
